...
describe("Endpoints clave unica", () => {
  describe("Generate state token and return client_id", () => {
    it("Should generate state token and return client_id", async (done) => {
      const response = await request.get("/v1/clave-unica/datos-conexion");
  ...
  describe("Generate sesion token from clave unica authentication", () => {
    it("Should not generate token from invalid state token", async (done) => {
      const response = await request.get(
        "/v1/clave-unica?code=c40a748e828843d48b8ae52dd9d3b238&state=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0b2tlbkNsYXZlVW5pY2EiOiJUb2tlbkNsYXZlVW5pY2EiLCJpYXQiOjE2MjAzOTk1NTEsImV4cCI6MTYyMDQwMDQ1MX0.o0-L5efmYHmefEIgT9LJJ-YWuWRYBL_Egve4u3myxm8"
      );
